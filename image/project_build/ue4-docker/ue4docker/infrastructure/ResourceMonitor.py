...
class ResourceMonitor(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            diskSpace = (
                formatSize(shutil.disk_usage(rootDir).free)
                if reportDisk == True
                else "Unknown"
            )
            ...
            self._logger.info(
                "[{}] [Available disk: {}] [Available memory: {} physical, {} virtual] [CPU usage: {:.2f}%]".format(
                    isoTime, diskSpace, physicalMemory, virtualMemory, cpu
                ),
                False,
            )
