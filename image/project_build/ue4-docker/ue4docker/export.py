from .infrastructure import DockerUtils, GlobalConfiguration, PrettyPrinting
from .exports import *
import sys
...
def export():
    ...
    COMPONENTS = {
        "installed": {
            ...
            "image": GlobalConfiguration.resolveTag("ue4-full"),
        ...
        "packages": {
            ...
            "image": GlobalConfiguration.resolveTag("ue4-full"),
            "help": "Runs a temporary conan server inside a container and uses it to export the\ngenerated conan-ue4cli wrapper packages.\n\n"
    ...
    if args["component"] is not None and args["component"] not in COMPONENTS:
        print(
            'Error: unrecognised component "{}".'.format(args["component"]),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Determine if we are displaying the help for a specific component
        ...
        print(
            "\nRun `{} export COMPONENT --help` for more information on a component.".format(
                sys.argv[0]
            )
        )
