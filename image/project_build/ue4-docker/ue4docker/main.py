...
def main():
    ...
    if installed == False:
        _exitWithError(
            "Error: could not detect Docker daemon version. Please ensure Docker is installed.\n\nError details: {}".format(
                error
            )
        )
    ...
    if platform.system() == "Windows":
        ...
        if host_build < min_build:
            _exitWithError(
                "Error: the detected build of Windows ({}) is not supported. {} or newer is required.".format(
                    host_build, min_build
                )
            )
    ...
    if platform.system() == "Darwin" and DarwinUtils.isSupportedMacOsVersion() == False:
        _exitWithError(
            "Error: the detected version of macOS ({}) is not supported. macOS {} or newer is required.".format(
                DarwinUtils.getMacOsVersion(), DarwinUtils.minimumRequiredVersion()
            )
        )
    ...
    if len(sys.argv) > 1:
        ...
        PrettyPrinting.printColumns(
            [(command, COMMANDS[command]["description"]) for command in COMMANDS]
        )
        print(
            "\nRun `{} COMMAND --help` for more information on a command.".format(
                sys.argv[0]
            )
        )
