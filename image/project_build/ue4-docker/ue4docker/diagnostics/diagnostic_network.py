...
class diagnosticNetwork(DiagnosticBase):
    ...
    def run(self, logger, args=[]):
        ...
        logger.action(
            "[network] Attempting to build an image that accesses network resources...",
            False,
        )
        built = self._buildDockerfile(
            logger, containerPlatform, diagnosticNetwork.IMAGE_TAG, buildArgs
        )

        # Inform the user of the outcome of the diagnostic
        if built == True:
            logger.action(
                "[network] Diagnostic succeeded! Running containers can access network resources without any issues.\n"
            )
        else:
            logger.error(
                "[network] Diagnostic failed! Running containers cannot access network resources. See the docs for troubleshooting tips:",
                True,
            )
            logger.error(
