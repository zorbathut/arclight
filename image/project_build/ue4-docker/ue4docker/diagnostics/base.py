...
class DiagnosticBase(object):
    ...
    def _checkPlatformMistmatch(self, logger, containerPlatform):
        ...
        if containerPlatform == "windows" and dockerPlatform == "linux":
            logger.error(
                "[{}] Error: attempting to test Windows containers while Docker Desktop is in Linux container mode.".format(
                    prefix
                ),
                False,
            )
            ...
            logger.error(
                "[{}] Error: attempting to test Linux containers while Docker Desktop is in Windows container mode.".format(
                    prefix
                ),
                False,
            )
    ...
    def _buildDockerfile(self, logger, containerPlatform, tag, buildArgs):
        ...
        self._printAndRun(
            logger, "[{}] ".format(prefix), ["docker", "system", "prune", "-f"]
        )
