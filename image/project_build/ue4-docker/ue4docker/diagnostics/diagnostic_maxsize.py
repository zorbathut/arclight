...
class diagnosticMaxSize(DiagnosticBase):
    ...
    def run(self, logger, args=[]):
        ...
        if hostPlatform != "windows":
            logger.info(
                "[maxsize] This diagnostic only applies to Windows host systems.", False
            )
            return True
        elif dockerPlatform != "windows":
            logger.error(
                "[maxsize] Error: Docker Desktop is currently in Linux container mode.",
                False,
            )
            logger.error(
                "[maxsize] Please switch to Windows container mode to run this diagnostic.",
                False,
            )
            return False
        ...
        if WindowsUtils.getWindowsBuild() < 18362:
            logger.info(
                "[maxsize] This diagnostic only applies to Windows Server and Windows 10 version 1903 and newer.",
                False,
            )
            return True
        ...
        try:
            ...
            logger.action(
                "[maxsize] Diagnostic succeeded! The host platform can specify maximum image sizes using Docker's `storage-opt` configuration key.\n"
            )
            return True
        except:
            logger.error(
                "[maxsize] Diagnostic failed! The host platform cannot specify maximum image sizes using Docker's `storage-opt` configuration key.\n",
                True,
            )
            return False
